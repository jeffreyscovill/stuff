...
function updateBatchHistory(batch_run_code, tgt, statuscode) {
    return snowflake.execute(
                  {
                  sqlText: "call curated.p_upd_batch_history(?,?,?)",
                  binds:[batch_run_code,tgt,statuscode]
                  }
                );
}
...
updateBatchHistory(BATCH_RUN_CODE, tablesourcemapid, ''0'');
msg += mergesql;
} catch (err) {
...
updateBatchHistory(BATCH_RUN_CODE, tablesourcemapid, ''1'');
snowflake.execute(
              {
